...
Os três polos estão localizado em: $[-82E3 -40 +40]$ , onde a função de transferência é descrita na Eq. \eqref{eq:dinamica:tfunc}, os parâmetros utilizados foram encontrados nas secções anteriores. \todo{atualizar}
...
Devido a não estabilidade do sistema em malha aberta e as alterações de ganho do atuador de forma não linear com a movimentação do rotor, optou-se por projetar um controlador com privilegiando a robustez a agressividade.
...
O controle obtido impõem um estabilidade no mancal com 60 graus de margem de fase e 27.8 dB margem de ganho. O tempo de acomodação é de 123 ms com sobressinal de 12 \%. A resposta ao pode ser visto na Fig. \ref{fig:controle:degrau}.

\begin{figure}[ht!]
\centering
\includegraphics[width=0.7\linewidth]{Figs/controle/step_pid_g}
\caption{Resposta ao degrau do sistema em malha fechada com o controlador PID para o sistema linearizado}
