...
A Fig. \ref{ativo_otimizado_fem_I_dx_map} ilustra a força resultante no rotor devido a aplicação de uma corrente nas bobinas primárias e secundárias. Valores positivos de força indicam que a maia atração é devido aos ímãs permanentes (estator externo) e uma força negativa, indica que a força resultante é devido a atração dos polos.

Verifica que o comportamento da força gerada é similar para os diferentes pontos de operação porém o ganho digital sofre alteração, inversamente proporcional ao tamanho do entreferro.

\begin{figure}[t!]
\centering
Força de atração (N) x Corrente (A)
\includegraphics[width=0.8\linewidth]{Figs/Simulacoes/Ativo/ativo_otimizado_fem_I_dx_map.pdf}
\caption{Força resultante da aplicação de uma corrente quando para diversos pontos de operação do rotor}
