...
O batente deve ser rígido suficiente para aguentar possíveis impactos do rotor, veremos no Cap. \ref{Cap:Modelagem:Dinamica} que essas forças são da ordem de centenas de Newtons.  \todo{Confuso e falta referencia} Propõem a utilização de Nylon ou Teflon \cite{Johnson2001a} na construção do batente devido as suas propriedades de lubrificação a seco. 
...
Nas simulações em elementos finitos, o modelo criado possui físicas distintas para os diversos elementos do mancal. No caso dos ferros, aplicou-se a lei de Ampère com a relação construtiva \textbf{BH} interpolada a uma  \todo{não precisa dizer qual tabela?}tabela. Nos componentes compostos por ar, a relação utilizada foi a linear $B=\mu_0 H$. 
...
Dificuldades foram enfrentadas com essa versão, dentre elas a do embobinamento dos polos, que não se podia realizar de forma manual pois a área dedicada ao embobinamento possuía valores próximos ao teórico  \todo{final confuso}para acomodar a quantidade de voltas calculadas, o que impossibilitou o com correntes.
