...
\noindent
\large{
	Esta dissertação vem ao encontro de projetar um mancal magnético para rodas de reação com aplicação na malha de controle de atitude de satélites. 
	...
	O mancal proposto possui dois graus de liberdade ativamente controlador (axial) e faz uso de ímãs para a estabilização passiva dos demais graus de liberdade. 
	
	Ao longo do desenvolvimento é apresentado modelos não lineares dos campos magnéticos e das forças atuantes no mancal. Com esses modelos, uma otimização é realizada a fim de encontrar melhores propriedades. Um modelo não linear da dinâmica do rotor é desenvolvido e um controle PID capaz de estabilizar os grau de liberdade ativo é apresentado.
}
...
This work comes to meet project a magnetic bearing for reaction wheels with application in satellite attitude control.

Magnetic bearings are alternatives to traditional bearings (balls, dry lubrication) because they work without mechanical contact between the rotor and the stator thereby minimizing friction between both parties. In addition to minimizing friction, the gain in reliability and lifetime of the reaction wheel is considerable by not present mechanical wear.

Because the consequences of any friction in the relative movement between the inertia (rotational part) and the satellite (stator), the bearing becomes a critical component of the reaction wheel. The friction is reflected not only in a greater consumption of electric power, as well as the introduction of a torque dead zone operation, and in limited life of the reaction wheel due to gradual wear of the bearing.

The proposed bearing has two degrees of freedom Actively controller (axial) and makes use of magnets for the passive stabilization of other degrees of freedom.

	
Throughout the development of this dissertation is presented nonlinear models of magnetic fields and forces acting on the bearing. With these models, an optimization is performed to find the best properties. A nonlinear model rotor dynamics is developed and a PID control capable of stabilizing the degree of freedom active produced
}
